# -*- coding: utf-8 -*-
"""
Created on Mon May 17 01:47:28 2021
...
class Population():
    ...
    def select(self, k=4): #k為幾個父母
        fit = self.evaluate()   #evaluate
        while len(self.parents) < k: 
            idx = np.random.randint(0, len(fit)) 
            if fit[idx] > np.random.rand(): #np.random.rand()?
                self.parents.append(self.bag[idx])
        self.parents = np.asarray(self.parents) #asarray不太會因站記憶體而對內容的更改產生問題 

    def mutate(self, p_cross=0.1, p_mut=0.1):
        next_bag = []
        children = self.crossover(p_cross) #crossover
...
if __name__ == "__main__":
    ...
    for V in range(4,21):
        for i in range(5):
            ...
            for i in range(1,V):
                vertex.append(i)
            graph_count = factorial(V)/(factorial(V-2)*factorial(2)) #這邊不懂
            first_graph = []
